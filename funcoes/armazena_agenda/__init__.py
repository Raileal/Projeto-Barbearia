...
class Armazenar_agenda:
    ...
    def inserir_data_hora_nome(self, data, hora, nome):
        cursor = self.db_connection.cursor()
        select_query = "SELECT COUNT(*) FROM barber_agenda WHERE data = %s AND hora = %s"
        cursor.execute(select_query, (data, hora))
        result = cursor.fetchone()
        if result[0] == 0:
            insert_query = "INSERT INTO barber_agenda (data, hora, nome) VALUES (%s, %s, %s)"
            cursor.execute(insert_query, (data, hora, nome))
            self.db_connection.commit()
        cursor.close()
    
    def atualizar_nome(self, data, hora, nome):
        cursor = self.db_connection.cursor()
        update_query = "UPDATE barber_agenda SET nome = %s WHERE data = %s AND hora = %s"
        cursor.execute(update_query, (nome, data, hora))
        self.db_connection.commit()
        cursor.close()
